...
import PhotoDeck from './PhotoDeck.jsx'
...
function About() {
  ...
  return (
      ...
      >
        ...
        {/* Block 3: text left, CERN photo deck right (back to default order) */}
        <div className={styles.row}>
          <div className={styles.text}>
            {about.cern.paragraphs.map((p, i) => (
                <p key={i} className={styles.paragraph}>{p}</p>
            ))}
          </div>
          <div className={styles.visual}>
            <PhotoDeck photos={about.cern.photos} />
          </div>
        </div>
      </section>
  )
